...
{
	if (DEBUG)
		std::cout << CYAN << "[REQUEST]" << RESET << " reading " << size << " bytes into file" << std::endl;
	if (!_outfile.is_open())
	...
	if (_state == READ_PLAIN_BODY && _body_size + static_cast<size_t>(size) > _content_length) {
		std::cout << BOLD_MAGENTA << "[FILE] " << _body_size + static_cast<size_t>(size) << " > " << _content_length << RESET << std::endl;
		size = _content_length - _body_size;
	}
...
{
	if (DEBUG)
		std::cout << CYAN << "[REQUEST]" << RESET << " Received bytes, buffer size=" << size << std::endl;
...
{
	if (DEBUG)
		std::cout << BOLD_CYAN << "[REQUEST]" << RESET << " creating temporary file for request body" << std::endl;
